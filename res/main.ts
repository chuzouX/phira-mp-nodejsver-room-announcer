...
const pluginModule: PluginModule = {
  ...
  async init(api: PluginApi) {
    // 读取配置
    const cfg = api.readPluginConfig<RoomAnnouncerConfig>() ?? {};
    ...
    // 获取房间状态文本
    function getRoomStateText(state: string): string {
      switch (state) {
        case 'SelectChart':
          return '选曲中';
    ...
    // 生成房间列表消息
    function generateRoomListMessage(): string | null {
      // 使用新的 API 获取房间列表并过滤
    ...
    // 获取未在房间中的玩家列表
    function getPlayersNotInRoom(): { id: number; name: string }[] {
      // 使用新的 API 获取在线玩家
    ...
    api.registerCommand('roomannouncer', (action?: string) => {
      if (action === 'status') {
        // 使用新的 API 获取服务器统计
